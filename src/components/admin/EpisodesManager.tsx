...
const EpisodesManager: React.FC = () => {
  ...
  // ✅ FIXED: fetchAnimes function with proper error handling
  const fetchAnimes = async () => {
    setAnimesLoading(true);
    try {
      const token = getToken();
      const response = await axios.get(`${API_BASE}/admin/protected/anime-list`, {
        headers: { 
      ...
      if (animeData && animeData.success !== undefined) {
        // If response has success property (like {success: true, data: [...]})
        if (animeData.success && Array.isArray(animeData.data)) {
          animeData = animeData.data;
        } else if (animeData.success && Array.isArray(animeData.animes)) {
          animeData = animeData.animes;
        }
      }
      
      // Ensure we have an array
      if (Array.isArray(animeData)) {
        const formattedAnimes = animeData.map((a: any) => ({
          ...a,
          id: a._id || a.id
        }));
        setAnimes(formattedAnimes);
      } else {
        console.error('❌ Unexpected anime data format:', animeData);
        setAnimes([]);
      }
    } catch (err: any) {
      console.error('❌ Animes load error:', err.response?.data || err.message);
      alert('Failed to load animes. Please check your connection or token.');
    } finally {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // If we had a selected anime, try to find it in the updated list
      if (selectedAnime) {
        const updatedSelectedAnime = animes.find((a: Anime) => a.id === selectedAnime.id);
        
        if (updatedSelectedAnime) {
          setSelectedAnime(updatedSelectedAnime);
          await fetchContent(updatedSelectedAnime.id);
        } else {
          // Selected anime might have been deleted
  ...
  const fetchContent = async (contentId: string) => {
    ...
    try {
      if (isManga) {
        const response = await axios.get(`${API_BASE}/chapters/${contentId}`);
        const data = response.data;
        
        // Handle different response structures
        let chaptersData = data;
        if (data && data.success !== undefined && data.success && Array.isArray(data.data)) {
        ...
        if (Array.isArray(chaptersData)) {
          ...
          console.error('❌ Unexpected chapters data:', data);
          setChapters([]);
        }
        
        // Set next chapter number
        const lastChapter = chaptersData.filter((ch: Chapter) => (ch.session || 1) === selectedSession);
        setNewItem(prev => ({
          ...prev,
        ...
        const response = await axios.get(`${API_BASE}/episodes/${contentId}`);
        const data = response.data;
        
        // Handle different response structures
        let episodesData = data;
        if (data && data.success !== undefined && data.success && Array.isArray(data.data)) {
        ...
        if (Array.isArray(episodesData)) {
          ...
          console.error('❌ Unexpected episodes data:', data);
          setEpisodes([]);
        }
        
        // Set next episode number
        const lastEpisode = episodesData.filter((ep: Episode) => (ep.session || 1) === selectedSession);
        setNewItem(prev => ({
          ...prev,
      ...
      alert(`Failed to load ${isManga ? 'chapters' : 'episodes'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddDownloadLinkEdit = () => {
  ...
  const handleRemoveDownloadLinkEdit = (index: number) => {
  ...
  const handleUpdateDownloadLinkEdit = (index: number, field: keyof DownloadLink, value: string) => {
    setEditForm(prev => ({
      ...prev,
      downloadLinks: prev.downloadLinks.map((link, i) => 
        i === index ? { ...link, [field]: value } : link
    ...
    try {
      ...
      const requestBody = isManga
        ? {
            ...
            title: newItem.title || `${isManga ? 'Chapter' : 'Episode'} ${newItem.number}`,
  ...
  const handleUpdateItem = async (itemId: string, itemNumber: number, session: number, e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      const requestBody = isManga
        ? {
            ...
            title: editForm.title || `${isManga ? 'Chapter' : 'Episode'} ${itemNumber}`,
      ...
      alert(err.response?.data?.error || `Failed to update ${isManga ? 'chapter' : 'episode'}`);
    }
  };
              ...
              <table className="w-full bg-slate-700/30 rounded-lg overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-700">
                  {filteredItems.map((item: any) => {
                    ...
                    const itemNumber = isManga ? item.chapterNumber : item.episodeNumber;
                          ...
                          <td className="p-3">
                            <div className="flex gap-2">
                              <button
                                ...
                                title="Edit"
                              >
                                {editingItemId === item._id ? 'Cancel Edit' : '✏️ Edit'}
                              </button>
                              <button
                                onClick={() => handleDeleteItem(item._id, itemNumber, item.session || 1)}
                              ...
                              <div className="animate-fade-in">
                                <div className="flex justify-between items-center mb-3">
                                  <h4 className="text-lg font-semibold text-white">
                                    Edit {isManga ? 'Chapter' : 'Episode'} {itemNumber}
                                    <span className="text-sm text-yellow-400 ml-2">(Session {item.session || 1})</span>
                                  </h4>
                                </div>
                                
                                <form onSubmit={(e) => handleUpdateItem(item._id, itemNumber, item.session || 1, e)} className="space-y-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      <label className="block text-sm font-medium text-slate-300 mb-2">Session *</label>
                                      <input
                                        type="number"
                                        ...
                                        className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors"
                                        min="1"
                                        required
                                      />
                                    </div>
                                  ...
                                  <div>
                                    <div className="flex justify-between items-center mb-2">
                                      ...
                                      <button
                                        type="button"
                                        onClick={handleAddDownloadLinkEdit}
                                        ...
                                        <div key={index} className="bg-slate-800/70 p-3 rounded-lg border border-slate-700">
                                          <div className="flex justify-between items-center mb-2">
                                            ...
                                            {editForm.downloadLinks.length > 1 && (
                                              <button
                                                type="button"
                                                onClick={() => handleRemoveDownloadLinkEdit(index)}
                                                className="text-xs bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded"
                                              >
                                                Remove
                                              </button>
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div>
                                              ...
                                              <input
                                                type="text"
                                                value={link.name}
                                                onChange={(e) => handleUpdateDownloadLinkEdit(index, 'name', e.target.value)}
                                                className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm"
                                                placeholder="e.g., Download Link 1, Server 1, etc."
                                                required
                                              />
                                            ...
                                            <div>
                                              ...
                                              <input
                                                type="text"
                                                value={link.quality || ''}
                                                onChange={(e) => handleUpdateDownloadLinkEdit(index, 'quality', e.target.value)}
                                                className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm"
                                                placeholder="e.g., 720p, HD, 1080p"
                                              />
                                            </div>
                                          ...
                                          <div className="mt-3">
                                            ...
                                            <input
                                              type="url"
                                              value={link.url}
                                              onChange={(e) => handleUpdateDownloadLinkEdit(index, 'url', e.target.value)}
                                              className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm"
                                              placeholder="https://example.com/download/video.mp4"
                                              required
                                            />
                                          ...
                                          <div className="mt-3">
                                            ...
                                            <select
                                              value={link.type || 'direct'}
                                              onChange={(e) => handleUpdateDownloadLinkEdit(index, 'type', e.target.value)}
                                              className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm"
                                            >
                                              <option value="direct">Direct Download</option>
                                              <option value="server">Server Download</option>
